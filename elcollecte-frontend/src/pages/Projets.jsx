  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        // dateDebut DOIT être une date valide (LocalDate côté backend)
        dateDebut: formData.dateDebut,
        // dateFin optionnel : null si vide
        dateFin: formData.dateFin || null,
        // zoneGeo : le backend attend Map<String,Object>, pas une String
        zoneGeo: formData.zoneGeo ? { nom: formData.zoneGeo } : null,
        statut: 'BROUILLON'
      };

      await client.post('/projets', payload);
      setShowCreateForm(false);
      setFormData({ titre: '', description: '', dateDebut: '', dateFin: '', zoneGeo: '' });
      fetchProjets();
    } catch (err) {
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(350px, 1fr))', gap: 20 }}>
          {filteredProjets.length === 0 ? (
              ...
              filteredProjets.map(projet => {
                ...
                // zoneGeo est un objet { nom: "..." } côté backend
              ...
              <div style={{
                ...
                {error && (
                    <div style={{
                      ...
                      background: '#fee2e2', color: '#991b1b', fontSize: '0.855rem'
                    }}>
                      {error}
                    </div>
                )}
